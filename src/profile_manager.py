...
class ProfileManager:
    ...
    def save_profile(self, name: str, description: str, monitors: dict[str, str]) -> None:
        """
    ...
    def apply_profile(self, name: str, controller: DisplayController, dry_run: bool = False) -> None:
        """
        ...
        if not monitors:
            click.echo(f"Error: Profile '{name}' has no monitor configuration", err=True)
            sys.exit(1)
        ...
        # Sort monitors in reverse order (highest display number first)
        # This prevents display numbering changes when lower displays disconnect temporarily
        sorted_monitors = sorted(monitors.items(), key=lambda x: int(x[0]), reverse=True)

        for idx, (display_num, input_name) in enumerate(sorted_monitors):
            input_name_lower = input_name.lower()
            if input_name_lower not in INPUT_MAP:
                click.echo(f"Warning: Invalid input '{input_name}' for display {display_num}, skipping", err=True)
                continue
            ...
            if current_input == input_code:
                click.echo(f"  Display {display_num} → {input_name.upper()} (code {input_code}) [already set, skipping]")
                continue

            click.echo(f"  Display {display_num} → {input_name.upper()} (code {input_code})")
